...
class BasicBlock(torch.nn.Module):
    def __init__(self,n_filters,reslu,fr_size):
        ...
        self.lambda_step = nn.Parameter(torch.Tensor([0.5]))#0.5,0,1,2
        self.soft_thr = nn.Parameter(torch.Tensor([0.01]))#0.01,1,2
    ...
    def forward(self, x, A,B,C,D,beta,y):
        ...
        # beta_pred[torch.abs(beta_pred) < 1 / 100] = torch.sgn(
        #     beta_pred[torch.abs(beta_pred) < 1 / 100]) * 1 / 100
