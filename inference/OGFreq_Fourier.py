...
class BasicBlock(torch.nn.Module):
    ...
    def forward(self, x,A,B,C,D,beta,y):
        ...
        x = x - self.lambda_step * (torch.matmul(A,(torch.matmul(C,x.transpose(-1,-2))+torch.matmul(D,(beta*x*self.reslu).transpose(-1,-2))).view(bsz,1,-1).transpose(-1,-2)).view(bsz,1,-1)
                                    +self.reslu*beta.conj()*torch.matmul(B,(torch.matmul(C,x.transpose(-1,-2))+torch.matmul(D,(beta*x*self.reslu).transpose(-1,-2))).view(bsz,1,-1).transpose(-1,-2)).view(bsz,1,-1))
        x = x + self.lambda_step * (torch.matmul(A,y.transpose(-1,-2)).view(bsz,1,-1)+self.reslu*beta.conj()*torch.matmul(B,y.transpose(-1,-2)).view(bsz,1,-1))
        ...
        beta_y=y-torch.matmul(C,x_pred.transpose(-1,-2)).view(bsz,1,-1)
        beta = beta - self.beta_lambda_step * (x_pred.conj()*self.reslu*torch.matmul(B,torch.matmul(D,(x_pred*beta*self.reslu).transpose(-1,-2)).view(bsz,1,-1).transpose(-1,-2)).view(bsz,1,-1))
        beta = beta + self.beta_lambda_step * (x_pred.conj()*self.reslu*torch.matmul(B,beta_y.transpose(-1,-2)).view(bsz,1,-1))
        beta_input = beta
...
class ISTANetplus_fourier(torch.nn.Module):
    def __init__(self, LayerNo,signal_dim,n_filters,inner,device,fr_size):
        super(ISTANetplus_fourier, self).__init__()
        onelayer = []
    ...
    def forward(self, y):
        ...
        C=torch.exp(2j * torch.pi / fr_size * torch.matmul(torch.arange(0, sig_dim)[:,None],torch.arange(-fr_size//2,fr_size//2)[None,:])).to(self.device)/(fr_size*sig_dim)
        D=2j * torch.pi / fr_size *torch.arange(0,sig_dim)[:,None].to(self.device)*C
        A=C.t().conj()
        B=D.t().conj()

        x = torch.matmul(A,y.transpose(-1,-2)).view(bsz, 1, -1)
        beta = torch.rand((bsz,1,self.inner)).to(self.device).type(torch.complex64)/2

        layers_sym = []   # for computing symmetric loss
        beta_layers_sym = []
        for i in range(self.LayerNo):
            [x, layer_sym,beta,beta_layer_sym] = self.fcs[i](x, A,B,C,D,beta,y)
