...
from OGFreq import ISTANetplus
from OGFreq_Fourier import ISTANetplus_fourier
from OGFreq_bn import ISTANetplus_bn
...
def load(fn, module_type, device = torch.device('cpu')):
    ...
    if device == torch.device('cpu'):
        args.use_cuda = False
    if module_type == 'skip1':
        model = complexModules.set_skip_module1(args)
    elif module_type == 'skip2':
        model = complexModules.set_skip_module2(args)
    elif module_type == 'skip3':
        model = complexModules.set_skip_module3(args)
    elif module_type == 'sbl':
        model = ISTANetplus(args.fr_n_layers, args.signal_dim, args.fr_n_filters, args.fr_inner_dim, device,
                                args.fr_size)
    elif module_type == 'sbl_fourier':
        model = ISTANetplus_fourier(args.fr_n_layers, args.signal_dim, args.fr_n_filters, args.fr_inner_dim, device,
                                args.fr_size)
    elif module_type == 'sbl_bn':
        model = ISTANetplus_bn(args.fr_n_layers, args.signal_dim, args.fr_n_filters, args.fr_inner_dim, device,
                                args.fr_size)
    elif module_type == 'sbl_conj':
        model = ISTANetplus_conj(args.fr_n_layers, args.signal_dim, args.fr_n_filters, args.fr_inner_dim, device,
                                args.fr_size)
    else:
