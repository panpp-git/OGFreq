...
import os
from ISTANet import ISTANetplus
...
if __name__ == '__main__':
    ...
    # basic parameters
    parser.add_argument('--output_dir', type=str, default='./checkpoint/init/train', help='output directory')
    ...
    parser.add_argument('--lr_fr', type=float, default=0.0012,
...
#    tb_writer = SummaryWriter(args.output_dir)
    ...
    # pre_path = 'checkpoint/damp/fr/epoch_140.pth'
    # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # fr_module, fr_optimizer, fr_scheduler, _, _ = util.load(pre_path, 'sbl', device)
    # fr_module.eval()

    device = torch.device("cuda:0" if args.use_cuda else "cpu")
    fr_module = ISTANetplus(args.fr_n_layers,args.signal_dim,args.fr_n_filters,args.fr_inner_dim,device,args.fr_size)
    fr_module = nn.DataParallel(fr_module)
    fr_module = fr_module.to(device)
    fr_optimizer, fr_scheduler = util.set_optim(args, fr_module, 'beam')
    start_epoch = 1
    ...
    for epoch in range(start_epoch, args.n_epochs_fr + 1):
        ...
        if epoch < args.n_epochs_fr:
            ...
            logger.info(
                "Epochs: %d / %d, Time: %.1f, FR training L2 loss %.2f, FR validation L2 loss %.2f, FNR %.2f %% (fr %.2f) (beta %.2f)",
                epoch, args.n_epochs_fr, time.time() - epoch_start_time, loss_train_total, loss_val_total,
                fnr_val, loss_train_fr1, loss_train_fr2)
